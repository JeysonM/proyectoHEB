class Service < ApplicationRecord
  ...
  after_save :save_image
    ...
    def image_filename
      File.join SERVICES, "service#{id+1}.gif"
    end

    def image_path
      "/image_store/service#{id}.gif"
    end
    ...
    def self.letter_search(letter)
        where(companies: {status: true}).where(companies: {status: true}).where("commercial_name LIKE '#{letter}%'")
    end
