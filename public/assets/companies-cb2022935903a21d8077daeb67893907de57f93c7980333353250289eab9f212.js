...
var CompaniesModel = function(_companies){
  ...
  function openCompanyToEdit(company_id) {
  ...
  function downCompany(){
    ...
    // company.downed_reason=self.editingReason;
    var token = localStorage.getItem("Token");
     ...
     $.ajax({
         ...
         success: function(returnedData){
            ...
            //  document.location.href="../../getDowned";
         },
  ...
  function createCompany() {
      if (!self.editingCompany().company.name) {
          self.errorMessages.push("Ingrese un usuario");
          self.hasError(true);
      }
      if (!self.editingCompany().company.email) {
          self.errorMessages.push("Ingrese un correo válido");
          self.hasError(true);
      }
      ...
      if (!self.editingCompany().company.nit) {
          self.errorMessages.push("Ingrese NIT de la empresa");
          self.hasError(true);
      }
      ...
      $.ajax({
          ...
          dataType: "json",
          success: function (returnedData) {
              alert("Muchas gracias, por registrar su empresa en nuestro catalogo, se le mandara un correo con la respuesta");
              self.companies.push(returnedData);
              document.location.href = "/";
          },
          error: function (error) {
              alert("Un error ha ocurrido al enviar su solicitud. " + error.message);
          }
      });
  }
  ...
  function createNewCompanyEditable(id,name,email,detail,contactName,address,telephone,webpage,department,subsidiaries){
    ...
    return newCompany;
     ...
     function createNewCompany(id,name,email,contactName,nit,address,telephone,status,department,webpage,logo_file_name,logo_file_size,logo_content_type,nitimage,created_at,detail, downed_reason,reason) {
...
var companiesModel;
(function() {
    var jqxhr = $.getJSON(serviceUrl+"companies", function(data) {
